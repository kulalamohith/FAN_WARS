...
export const api = {
  auth: {
    login: (phone: string) =>
      request<{ is_new_user: boolean; token?: string; user?: any; phone?: string }>('/auth/login', {
        method: 'POST', body: JSON.stringify({ phone }),
      }),
    onboard: (phone: string, username: string, armyId: string) =>
      request<{ token: string; user: any }>('/auth/onboard', {
        method: 'POST', body: JSON.stringify({ phone, username, armyId }),
      }),
    me: () =>
